...
function processImage(img: HTMLImageElement): ImageData {
 ...
 const idx = y * width + x;
 const sm = shapeMask[idx] ?? 0;
    if (!sm) continue;
 const left = x > 0 ? shapeMask[idx - 1] : 0;
 const right = x < width - 1 ? shapeMask[idx + 1] : 0;
 const up = y > 0 ? shapeMask[idx - width] : 0;
 const down = y < height - 1 ? shapeMask[idx + width] : 0;
 if (
 x === 0 ||
 x === width - 1 ||
 y === 0 ||
 ...
 const idx = y * width + x;
 if (!(shapeMask[idx] ?? 0) || (boundaryMask[idx] ?? 0)) continue;
    const sRight = (shapeMask[idx + 1] ?? 0) ? (u[idx + 1] ?? 0) : 0;
    const sLeft = (shapeMask[idx - 1] ?? 0) ? (u[idx - 1] ?? 0) : 0;
    const sDown = (shapeMask[idx + width] ?? 0) ? (u[idx + width] ?? 0) : 0;
    const sUp = (shapeMask[idx - width] ?? 0) ? (u[idx - width] ?? 0) : 0;
    const sum = sRight + sLeft + sDown + sUp;
 ...
 let maxVal = 0;
 for (let i = 0; i < size; i++) { const val = u[i] ?? 0; if (val > maxVal) maxVal = val; }
...
export default function MetallicPaint({
 ...
 if (info && info.name) uniforms[info.name] = gl.getUniformLocation(prog, info.name);
 }
 ...
 const uTex = uniforms['u_tex'];
 if (uTex) gl.uniform1i(uTex, 0);

 const ratio = imgData.width / imgData.height;
 const uImgRatio = uniforms['u_imgRatio'];
 const uRatio = uniforms['u_ratio'];
 if (uImgRatio) gl.uniform1f(uImgRatio, ratio);
 ...
 const setUniform3f = (name: string, v1: number, v2: number, v3: number) => {
 const loc = u[name];
 if (loc) gl.uniform3f(loc, v1, v2, v3);
 };

 setUniform1f('u_seed', seed);
 setUniform1f('u_scale', scale);
 setUniform1f('u_refract', refraction);
 setUniform1f('u_blur', blur);
 setUniform1f('u_liquid', liquid);
 setUniform1f('u_bright', brightness);
 setUniform1f('u_contrast', contrast);
 setUniform1f('u_angle', angle);
 setUniform1f('u_fresnel', fresnel);

 const light = hexToRgb(lightColor);
 const dark = hexToRgb(darkColor);
 const tint = hexToRgb(tintColor);
 setUniform3f('u_lightColor', light[0], light[1], light[2]);
 setUniform3f('u_darkColor', dark[0], dark[1], dark[2]);
 setUniform1f('u_sharp', patternSharpness);
 setUniform1f('u_wave', waveAmplitude);
 setUniform1f('u_noise', noiseScale);
 setUniform1f('u_chroma', chromaticSpread);
 setUniform1f('u_distort', distortion);
 setUniform1f('u_contour', contour);
 setUniform3f('u_tint', tint[0], tint[1], tint[2]);
 }, [
 ready,
 seed,
 scale,
 ...
 if (uTime) gl.uniform1f(uTime, animTimeRef.current);
 ...
 return <canvas ref={canvasRef} className={`block h-full w-full object-contain ${className}`} />;
}
