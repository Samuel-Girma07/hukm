...
function buildBoxShadow(glowColor: string, intensity: number): string {
 ...
 return layers.map(([x, y, blur, spread, alphaVal, inset]) => {
 const a = Math.min((alphaVal as number) * intensity, 100);
...
const BorderGlow: React.FC<BorderGlowProps> = ({
 ...
 useEffect(() => {
 if (focused) {
  // steady glow intensity while focused
  setEdgeProximity(0.6);
  // start slow continuous rotation
  const rotate = (t: number) => {
   const angle = (t * 0.03) % 360;
   focusAngleRef.current = angle;
   setCursorAngle(angle);
   focusAnimRef.current = requestAnimationFrame(rotate);
  };
  focusAnimRef.current = requestAnimationFrame(rotate);
 } else {
 ...
 const isVisible = isHovered || sweepActive || focused;
 const effectiveEdgeProximity = focused && !isHovered ? 0.6 : edgeProximity;
 ...
 {/* Children wrapper — overflow-visible so dropdowns can escape the card bounds */}
 <div className="flex flex-col relative overflow-visible z-[1]">
 {children}
 </div>
 </div>
 );
